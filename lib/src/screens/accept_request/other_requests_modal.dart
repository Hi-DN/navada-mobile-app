...
class OtherRequestsModal extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    _context = context;
    Provider.of<AcceptRequestProvider>(_context!, listen: false).loadMore(false);
    ScreenSize size = ScreenSize();
    return Container(
      padding: EdgeInsets.only(top: size.getSize(15), left: size.getSize(15), right: size.getSize(15)),
  ...
  Widget _otherRequestListSection() {
    ScreenSize size = ScreenSize();
    List<RequestDto> requests = Provider.of<AcceptRequestProvider>(_context!).otherRequests;
    ...
    ? NotificationListener<ScrollNotification>(
      ...
      child: RefreshIndicator(
        ...
        onRefresh: () async {
          Provider.of<AcceptRequestProvider>(_context!, listen: false).loadMore(true);
        },
          ...
          Column(
            ...
            Row(
              children: [
                ...
                R14Text(
                  text: ' | ${Shortener.shortenStrTo(product.userNickname!, 4)}',
                  textColor: grey153)]),
            Row(
                children: [
  ...
  Widget _goAcceptRequestBtn(RequestDto request) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(_context!,
              MaterialPageRoute(builder: (BuildContext context) => AcceptRequestView(request: request))),
      child: Icon(Icons.arrow_forward_ios, color: grey183, size: size.getSize(18)));
  }
}
