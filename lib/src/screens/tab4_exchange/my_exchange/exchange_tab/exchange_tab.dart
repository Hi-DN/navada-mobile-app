...
import 'exchange_detail/exchange_detail_view.dart';
    ...
    return Consumer<MyExchangesExchangeProvider>(builder: (BuildContext context,
      ...
      switch (provider.dataState) {
        ...
        case DataState.REFRESHING:
          return _ExchangeListView(
              exchangeList: provider.exchangeDataList, isLoading: true);
...
class _ExchangeListView extends StatelessWidget {
  _ExchangeListView(
      {Key? key, required this.exchangeList, required this.isLoading})
      : super(key: key);
    ...
    return Expanded(
      child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            bool isTrading =
                (exchange.exchangeStatusCd == ExchangeStatusCd.TRADING);
  ...
  Widget _tappableTradingExchangeItem(
      BuildContext? context, ExchangeDtoModel? exchange) {
  ...
  Widget _dismissibleCompletedExchangeItem(
      BuildContext? context, ExchangeDtoModel? exchange) {
    ScreenSize size = ScreenSize();
  ...
  Widget _filterSection() {
    int totalElements =
        Provider.of<MyExchangesExchangeProvider>(_context!).totalElements;
    return Column(
      children: [
        Row(
    ...
    return MyExchangeCard(
      statusSign: isTrading
          ? const MyExchangeStatusSign(
              color: green, icon: Icons.compare_arrows, label: '교환중')
          : const MyExchangeStatusSign(
      ...
      params: MyExchangeCardParams(
          ...
          requesterNickname: Row(children: [
            ...
            SizedBox(
                ...
                child: R10Text(
                    text: requesterProduct?.userNickname, textColor: grey183)),
          ]),
          ...
          acceptorNickname: Row(children: [
            ...
            SizedBox(
                width: size.getSize(66),
                child: R10Text(
                    text: acceptorProduct?.userNickname, textColor: grey183)),
          ]),
...
class _ViewFilter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    _context = context;
    ScreenSize size = ScreenSize();
    MyExchangesFilterItem curFilter =
        Provider.of<MyExchangesViewModel>(_context!).curFilter;
