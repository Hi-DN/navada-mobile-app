...
import 'exchange_confirm_modal.dart';
import 'exchange_detail_view_model.dart';
...
class ExchangeDetailView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool isCompleteFeatureActive = !(isAcceptor
        ? exchange!.acceptorConfirmYn!
        : exchange!.requesterConfirmYn!);

    return MultiProvider(
        providers: [
          ChangeNotifierProvider(
              create: (context) =>
                  ExchangeDetailViewModel(isCompleteFeatureActive)),
        ],
        child: Consumer<ExchangeDetailViewModel>(builder: (BuildContext context,
            ExchangeDetailViewModel viewModel, Widget? _) {
          bool isCompleteBtnActive = viewModel.isCompleteFeatureActive;

          return Scaffold(
            appBar: CustomAppBar(
              titleText:
                  exchange!.exchangeStatusCd! == ExchangeStatusCd.COMPLETED
                      ? "교환완료"
                      : "교환중",
              leadingYn: true,
            ...
            body: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  _ProductInfo(
                      product: exchange!.requesterProduct, isAcceptor: false),
                  _ratingSection(exchange!.requesterProduct!.userNickname!,
                      exchange!.acceptorRating!, exchange!.requesterConfirmYn!),
                  const Space(height: 20),
                  const CustomDivider(),
                  _ProductInfo(
                      product: exchange!.acceptorProduct, isAcceptor: true),
                  _ratingSection(exchange!.acceptorProduct!.userNickname!,
                      exchange!.requesterRating!, exchange!.acceptorConfirmYn!),
                  const Space(height: 20),
                  _warningSection(),
                  Row(
                    ...
                    children: [
                      LongCircledBtn(
                        text: "교환 완료하기",
                        onTap: () => isCompleteBtnActive
                            ? _showExchangeConfirmModal(context)
                            : null,
  ...
  _showExchangeConfirmModal(BuildContext? context) {
    ...
    final viewModel =
        Provider.of<ExchangeDetailViewModel>(context!, listen: false);
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return ChangeNotifierProvider.value(
            value: viewModel, child: ExchangeConfirmModal(exchange: exchange));
      },
    );
  }
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const R16Text(text: "님이 준 별점")
          ]),
          const Space(height: 15),
          (rating < 0)
              ? R16Text(
                  text: isComplete
                      ? "😀교환 완료시 별점을 주지 않으셨습니다!"
                      : "아직 별점을 입력하지 않으셨습니다.",
                  textColor: grey153)
...
class _ProductInfo extends StatelessWidget {
  const _ProductInfo({Key? key, this.product, this.isAcceptor})
      : super(key: key);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _imageAndDetailSection(product?.productImageUrl),
  ...
  Widget _imageAndDetailSection(String? productImageUrl) {
    return Row(
      children: [
